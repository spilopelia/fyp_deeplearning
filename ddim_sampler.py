...
def extract(v, i, shape):
    ...
    """
    v = v.to(device=i.device, dtype=torch.float32)
...
class DDPMSampler(nn.Module):
    ...
    def cal_mean_variance(self, x_t, t):
        ...
        mean = extract(self.coeff_1, t, x_t.shape) * x_t[:,0:3,:,:,:] - extract(self.coeff_2, t, x_t.shape) * epsilon_theta
    ...
    def sample_one_step(self, x_t, time_step: int):
        ...
        z = torch.randn_like(x_t[:,0:3,:,:,:]) if time_step > 0 else 0
    ...
    @torch.no_grad()
    def forward(self, x_t, input_data, only_return_x_0: bool = True, interval: int = 1, **kwargs):
        """
        Parameters:
        ...
        """
        x_t = torch.cat([x_t, input_data], dim=1)
        x = [x_t]
        ...
        if only_return_x_0:
            return x_t[:,0:3,:,:,:]  # [batch_size, channels, height, width]
        return torch.stack(x[:,0:3,:,:,:], dim=1)  # [batch_size, sample, channels, height, width]
...
class DDIMSampler(nn.Module):
    ...
    def sample_one_step(self, x_t, time_step: int, prev_time_step: int, eta: float):
        ...
        alpha_t = extract(self.alpha_t_bar, t, x_t[:,0:3,:,:,:].shape)
        alpha_t_prev = extract(self.alpha_t_bar, prev_t, x_t[:,0:3,:,:,:].shape)
        ...
        epsilon_t = torch.randn_like(x_t[:,0:3,:,:,:])
        x_t_minus_one = (
                torch.sqrt(alpha_t_prev / alpha_t) * x_t[:,0:3,:,:,:] +
    ...
    @torch.no_grad()
    def forward(self, x_t, input_data, steps: int = 1, method="linear", eta=0.0,
